...
interface UseGeminiReturn {
  askText:      (text: string) => Promise<AIResponse | null>;
  isLoading:    boolean;
  clearHistory: () => void;
}
...
export function useGemini({ apiKey, userName }: UseGeminiOptions): UseGeminiReturn {
  ...
  const askText = useCallback(async (userText: string): Promise<AIResponse | null> => {
    ...
    try {
      ...
      historyRef.current.pop(); // revertir turno fallido
      return {
        action: 'unknown',
        value:  null,
  ...
  return { askText, isLoading, clearHistory };
}
